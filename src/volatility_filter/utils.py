...
def format_trade_data(trade: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        'trade_id': trade.get('trade_id', f"{trade.get('timestamp')}_{trade.get('trade_seq', '')}"),
...
def calculate_price_statistics(trades: List[Dict[str, Any]]) -> Dict[str, float]:
    ...
    vwap = sum(p * a for p, a in zip(prices, amounts)) / sum(amounts) if sum(amounts) > 0 else 0
...
def create_time_buckets(trades: List[Dict[str, Any]], bucket_size: str = '1H') -> pd.DataFrame:
    """
...
def calculate_trade_intensity(trades: List[Dict[str, Any]], 
                            time_window: int = 300000) -> List[Tuple[int, int]]:
    """
    ...
    while current_window_start <= sorted_trades[-1]['timestamp']:
        ...
        count = sum(1 for t in sorted_trades 
