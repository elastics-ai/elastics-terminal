...
class OptionChainManager:
    ...
    async def fetch_and_store_instruments(self):
        ...
        try:
            instruments = self.option_fetcher.fetch_option_instruments(self.currency, expired=False)
    ...
    async def process_ticker_update(self, channel: str, data: Dict):
        ...
        try:
            ...
            if instrument_name not in self.option_prices or \
               time.time() - self.option_prices[instrument_name].get('last_db_update', 0) > 60:
    ...
    async def periodic_surface_fitting(self):
        ...
        while True:
            try:
                ...
                if len(self.option_prices) >= self.min_options_for_fit and self.spot_price:
        ...
        try:
            ...
            for instrument_name, price_data in self.option_prices.items():
                ...
                opt_data = OptionData(
                    ...
                    expiry=datetime.fromtimestamp(instrument['expiry_timestamp']/1000),
            ...
            if len(option_data_list) < self.min_options_for_fit:
                logger.warning(f"Insufficient options for surface fit: {len(option_data_list)}")
                return None
                
            # Fit the surface
            surface_result = self.surface_fitter.fit_surface(option_data_list, current_time)
            ...
            if surface_result:
                ...
                logger.info(f"Fitted volatility surface with {surface_result['num_options']} options")
    ...
    async def periodic_status_log(self):
        ...
        while True:
            ...
            active_options = len([p for p in self.option_prices.values() if p.get('mark_iv')])
            logger.info(f"Option Chain Manager Status - Messages: {self.message_count}, "
    ...
    def calculate_black_scholes_iv(self, option_price: float, spot: float, strike: float,
                                  time_to_expiry: float, risk_free_rate: float = 0.0,
