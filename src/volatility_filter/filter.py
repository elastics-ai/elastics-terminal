...
class DeribitVolatilityFilter:
    ...
    def __init__(self, 
        ...
        if broadcast_events:
            self.broadcast_server = WebSocketBroadcastServer(broadcast_host, broadcast_port)
        ...
        logger.info(f"Volatility filter initialized with threshold={vol_threshold:.4f}, "
    ...
    def save_config(self):
        ...
        if self.db_manager:
            config_data = {
                ...
                'optimization_method': getattr(self.optimizer, 'optimization_results', {}).get('method'),
                'optimization_score': getattr(self.optimizer, 'optimization_results', {}).get('best_f1_score')
            }
    ...
    def process_trade(self, trade: Dict[str, Any]):
        ...
        trade_data = {
            ...
            'trade_id': trade.get('trade_id', f"{timestamp}_{trade.get('trade_seq', '')}")
        }
    ...
    def on_filtered_trade(self, trade: Dict[str, Any]):
        ...
        logger.info(f"Filtered Trade: {trade['datetime']} | "
    ...
    def optimize_threshold_on_startup(self):
        ...
        if len(trades) < 1000:
            logger.warning(f"Only {len(trades)} trades fetched. Optimization may not be reliable.")
            return
    ...
    def load_optimized_threshold(self, filename: str = 'vol_threshold_optimization.pkl') -> bool:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            'overall_ratio': self.total_events_detected / self.total_trades_processed if self.total_trades_processed > 0 else 0
        }
...
def main():
    ...
    parser.add_argument('--threshold', type=float, default=0.015, help='Volatility threshold')
