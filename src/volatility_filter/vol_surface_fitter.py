...
class VolatilitySurfaceFitter:
    ...
    def calculate_time_to_expiry(self, expiry: datetime, current_time: datetime) -> float:
        """Calculate time to expiry in years"""
        time_diff = expiry - current_time
        return time_diff.total_seconds() / (365.25 * 24 * 3600)
    
    def fit_surface(self, option_data: List[OptionData], current_time: Optional[datetime] = None) -> Dict:
        """
        ...
        valid_options = [opt for opt in option_data if opt.implied_volatility is not None and opt.implied_volatility > 0]
        
        if len(valid_options) < 10:
            logger.warning(f"Insufficient valid options for surface fitting: {len(valid_options)}")
            return None
        ...
        try:
            rbf = interpolate.Rbf(moneyness, ttm, ivs, function='thin_plate', smooth=0.1)
            ...
            logger.info(f"Successfully fitted volatility surface with {len(valid_options)} options")
            return result
    ...
    def interpolate_vol(self, strike: float, expiry: datetime, spot: float, 
        ...
        # Check bounds
        if (m < self.strikes.min() or m > self.strikes.max() or 
            t < self.expiries.min() or t > self.expiries.max()):
            logger.warning(f"Interpolation point outside surface bounds: m={m:.3f}, t={t:.3f}")
            return None
            
        # Bilinear interpolation
        interp = interpolate.RegularGridInterpolator((self.expiries, self.strikes), self.surface)
