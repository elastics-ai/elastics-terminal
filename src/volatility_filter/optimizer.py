...
class VolatilityFilterOptimizer:
    ...
    def prepare_historical_data(self, trades: List[Dict[str, Any]]) -> pd.DataFrame:
        ...
        df['high_vol'] = df['realized_vol'] > df['realized_vol'].quantile(vol_threshold_percentile/100)
    ...
    def calculate_ar_volatility(self, returns: np.ndarray, min_size: int = 20) -> List[float]:
        """
    ...
    def objective_function(self, threshold: float, returns: np.ndarray, ground_truth: np.ndarray) -> float:
        """
    ...
    def optimize_threshold(self, method: str = 'grid_search') -> float:
        ...
        if self.historical_data is None:
            raise ValueError("No historical data available. Call prepare_historical_data first.")
        ...
        if method == 'grid_search':
            ...
            for i, threshold in enumerate(thresholds):
                ...
                if i % 20 == 0:
                    logger.debug(f"Grid search progress: {i/len(thresholds)*100:.0f}%")
        ...
        logger.info(f"Optimization complete. Best threshold: {best_threshold:.4f}, F1 score: {best_score:.3f}")
    ...
    def backtest_threshold(self, threshold: float) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            'positive_rate': np.sum(predictions) / len(predictions) if len(predictions) > 0 else 0
        }
    ...
    def save_optimization_results(self, filename: str = 'vol_threshold_optimization.pkl'):
    ...
    def load_optimization_results(self, filename: str = 'vol_threshold_optimization.pkl') -> Dict[str, Any]:
