...
class HistoricalDataFetcher:
    ...
    def fetch_trades(self, 
                    ...
                    count: int = 1000,
                    max_trades: int = 10000) -> List[Dict[str, Any]]:
        """
        ...
        logger.info(f"Fetching trades for {instrument} from {datetime.fromtimestamp(start_timestamp/1000)} "
        ...
        while current_end > start_timestamp and len(trades) < max_trades:
            ...
            try:
                ...
                if 'result' in data and 'trades' in data['result']:
                    ...
                    logger.debug(f"Fetched {len(new_trades)} trades (total: {len(trades)})")
        ...
        logger.info(f"Fetched {len(trades_filtered)} trades in {request_count} requests")
        return trades_filtered
        
    def fetch_trades_by_date_range(self,
                                  instrument: str = "BTC-PERPETUAL",
    ...
    def fetch_instrument_info(self, instrument: str = "BTC-PERPETUAL") -> Optional[Dict[str, Any]]:
        """
    ...
    def fetch_order_book(self, instrument: str = "BTC-PERPETUAL", depth: int = 10) -> Optional[Dict[str, Any]]:
        """
