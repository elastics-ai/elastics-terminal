...
class OptionDataFetcher:
    ...
    def fetch_option_instruments(self, currency: str = "BTC", expired: bool = False) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if 'result' in data:
                ...
                logger.info(f"Fetched {len(parsed_instruments)} option instruments for {currency}")
    ...
    def fetch_option_chain(self, currency: str = "BTC", expiry_dates: List[str] = None) -> Dict[str, Any]:
        """
        ...
        if expiry_dates:
            ...
            for date_str in expiry_dates:
                try:
                    # Parse DMMMYY format
                    dt = datetime.strptime(date_str, "%d%b%y").replace(hour=8)  # 8 AM UTC expiry
                    expiry_timestamps.add(int(dt.timestamp() * 1000))
                except Exception as e:
                    logger.error(f"Error parsing expiry date {date_str}: {e}")
                    
            instruments = [i for i in instruments if i['expiry_timestamp'] in expiry_timestamps]
        ...
        for inst in instruments:
            ...
            chain_data['expiries'][expiry_date]['strikes'][strike][inst['option_type']] = inst
        
        logger.info(f"Fetched option chain with {len(chain_data['expiries'])} expiries")
        return chain_data
        
    def fetch_option_order_book(self, instrument_name: str, depth: int = 10) -> Optional[Dict[str, Any]]:
        """
    ...
    def fetch_greeks_for_instruments(self, instrument_names: List[str]) -> Dict[str, Dict[str, Any]]:
        """
    ...
    def fetch_volatility_index(self, currency: str = "BTC", resolution: str = "1h") -> Optional[Dict[str, Any]]:
        """
