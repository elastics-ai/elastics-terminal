...
def demonstrate_option_data_fetching():
    ...
    if instruments:
        ...
        for inst in instruments[:5]:
            exp_date = datetime.fromtimestamp(inst["expiry_timestamp"] / 1000).strftime(
                "%Y-%m-%d"
            )
            print(
                f"   - {inst['instrument_name']}: Strike ${inst['strike']:,.0f}, "
...
def demonstrate_option_filter():
    ...
    try:
        ...
        while True:
            ...
            print(
                f"Underlying price: ${stats['underlying_price']:,.2f}"
                if stats["underlying_price"]
                else "Underlying price: N/A"
            )
...
def demonstrate_database_queries():
    ...
    if not trades.empty:
        ...
        for _, trade in trades.head(5).iterrows():
            print(
                f"   - {trade['instrument_name']}: ${trade['price']} "
...
async def demonstrate_vol_surface_tui():
    ...
    class VolSurfaceScreen(Screen):
        ...
        def compose(self) -> ComposeResult:
            ...
            yield Container(
                Static(id="surface-display", expand=True), id="surface-container"
            )
            yield Container(
                Vertical(
        ...
        async def connect_websocket(self) -> None:
            ...
            try:
                async with websockets.connect(uri) as websocket:
                    ...
                    await websocket.send(
                        json.dumps({"type": "subscribe", "events": ["vol_surface"]})
                    )
        ...
        def update_display(self) -> None:
            ...
            # Update stats
            self.query_one("#spot-price").update(
                f"Spot Price: ${self.surface_data['spot_price']:,.2f}"
            )
            self.query_one("#atm-vol").update(
                f"ATM Volatility: {self.surface_data['atm_vol']:.1%}"
            )
            self.query_one("#last-update").update(
                f"Last Update: {datetime.now().strftime('%H:%M:%S')}"
            )
            self.query_one("#num-options").update(
                f"Options Used: {self.surface_data['num_options']}"
            )
