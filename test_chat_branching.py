...
def test_chat_branching():
    ...
    response = requests.get(f"{BASE_URL}/api/chat/conversations/{conversation_id}/messages")
    ...
    print(f"Will branch from message {branch_from_id}: '{user_message['content'][:50]}...'")
    ...
    for branch in branches:
        print(f"  - Branch {branch['id']}: {branch['title']} ({branch['message_count']} messages)")
    ...
    def print_tree(node, indent=0):
        prefix = "  " * indent
        print(f"{prefix}- {node['title']} (ID: {node['id']}, Messages: {node.get('message_count', 0)})")
