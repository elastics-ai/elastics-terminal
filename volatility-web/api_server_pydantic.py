"""FastAPI backend for volatility terminal web app with Pydantic models."""
...
from src.volatility_filter.models import (
    # Portfolio models
    Position, PortfolioSummary, PnLBreakdown,
    # Chat models
    ChatMessage, ChatResponse, Conversation, ConversationCreate, 
    ConversationUpdate, BranchCreate, ChatSuggestion,
    # Market models
    PolymarketMarket, PolymarketResponse, MarketStats,
    # Volatility models
    VolatilityAlert, VolatilitySurface,
    # SQL models
    SQLModule, SQLModuleUpdate, SQLExecutionResult, SQLModulesResponse, SQLModuleStats,
)
...
async def get_portfolio_positions() -> List[Position]:
...
@app.get("/api/portfolio/pnl-breakdown", response_model=PnLBreakdown)
async def get_pnl_breakdown() -> PnLBreakdown:
...
@app.get("/api/volatility/alerts", response_model=List[VolatilityAlert])
async def get_volatility_alerts(limit: int = 10) -> List[VolatilityAlert]:
    ...
    async with get_db() as conn:
        ...
        alerts = []
        async for row in cursor:
            alerts.append(VolatilityAlert(**dict(row)))

        return alerts


@app.get("/api/volatility/surface/latest", response_model=VolatilitySurface)
async def get_latest_vol_surface() -> VolatilitySurface:
    ...
    async with get_db() as conn:
        ...
        if row:
            ...
            return VolatilitySurface(**result)
        else:
            raise HTTPException(
...
@app.get("/api/polymarket/markets", response_model=PolymarketResponse)
async def get_polymarket_markets(
    ...
    try:
        ...
        markets_data = await polymarket_client.get_markets(
    ...
    return PolymarketResponse(
        ...
        total=len(markets),
        last_update=datetime.now(),
        is_mock=not markets_data or markets == demo_markets,
    )
...
@app.get("/api/chat/suggestions", response_model=ChatSuggestion)
async def get_chat_suggestions() -> ChatSuggestion:
    ...
    try:
        async with get_db() as conn:
            ...
            return ChatSuggestion(suggestions=suggestions)
    except Exception as e:
...
async def get_conversation_details(conversation_id: int) -> Conversation:
...
async def create_conversation(conversation_data: ConversationCreate):
    ...
    try:
        conversation_id = db_manager.create_chat_conversation(conversation_data.dict())
...
async def create_conversation_branch(conversation_id: int, branch_data: BranchCreate):
    ...
    try:
        ...
        for msg in all_messages:
            ...
            if msg["id"] == branch_data.parent_message_id:
                break
        ...
        new_conversation_data = {
            ...
            "title": branch_data.title or f"Branch of {parent_conversation.get('title', 'Conversation')}",
        ...
        return {
            ...
            "branch_point": branch_data.parent_message_id,
        }
    except Exception as e:
...
async def update_conversation(conversation_id: int, update_data: ConversationUpdate):
    ...
    try:
        success = db_manager.update_chat_conversation(
            conversation_id, title=update_data.title, use_case=update_data.use_case
        )
...
@app.get("/api/stats/realtime", response_model=MarketStats)
async def get_realtime_stats() -> MarketStats:
...
@app.get("/api/modules", response_model=SQLModulesResponse)
async def get_sql_modules(
    ...
    try:
        modules_data = db_manager.get_sql_modules(
            limit=limit,
            offset=offset,
            search=search,
        ...
        modules = [SQLModule(**m) for m in modules_data]
        
        return SQLModulesResponse(
            modules=modules,
            total=total_count,
            limit=limit,
            offset=offset,
...
@app.get("/api/modules/{module_id}", response_model=SQLModule)
async def get_sql_module(module_id: int) -> SQLModule:
    """Get a specific SQL module by ID."""
    try:
        module = db_manager.get_sql_module_by_id(module_id)
        if not module:
            raise HTTPException(status_code=404, detail="Module not found")
        return SQLModule(**module)
    except Exception as e:
...
@app.post("/api/modules/{module_id}/execute", response_model=SQLExecutionResult)
async def execute_sql_module(module_id: int) -> SQLExecutionResult:
...
async def update_sql_module(module_id: int, update_data: SQLModuleUpdate):
...
@app.get("/api/modules/stats/overview", response_model=SQLModuleStats)
async def get_modules_stats() -> SQLModuleStats:
    """Get overall SQL modules statistics."""
    try:
        stats = db_manager.get_sql_module_stats()
        return SQLModuleStats(**stats)
    except Exception as e:
