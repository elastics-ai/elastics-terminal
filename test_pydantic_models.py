#!/usr/bin/env python3
"""Test script for Pydantic models implementation."""
...
from src.volatility_filter.models import (
    Position, PortfolioSummary, ChatMessage, PolymarketMarket,
    VolatilityAlert, SQLModule, InstrumentType
)
...
def test_portfolio_models():
    ...
    print(f"Position JSON: {position.model_dump_json()}")
    ...
    print(f"\nPortfolio Summary: {summary.model_dump()}")
...
def test_chat_models():
    ...
    print(f"Chat Message: {message.model_dump()}")
...
def test_market_models():
    ...
    print(f"Polymarket: {market.model_dump()}")
...
def test_volatility_models():
    ...
    print(f"Volatility Alert: {alert.model_dump()}")
...
def test_sql_models():
    ...
    print(f"SQL Module: {module.model_dump()}")
...
def test_validation():
    ...
    try:
        ...
        print(f"✓ Valid position created with enum: {position.type}")
    except Exception as e:
