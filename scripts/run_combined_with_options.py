...
class CombinedVolatilitySystem:
    ...
    async def on_surface_update(self, surface_data):
        ...
        logger.info(f"Volatility surface updated with {surface_data.get('num_options', 0)} options")
...
def main():
    parser = argparse.ArgumentParser(description='Run Deribit Volatility Filter with Option Chain Integration')
    parser.add_argument('--optimize', action='store_true', help='Optimize threshold before starting')
    parser.add_argument('--window', type=int, help='Window size for rolling calculations')
    ...
    parser.add_argument('--no-broadcast', action='store_true', help='Disable WebSocket broadcast')
    ...
    parser.add_argument('--option-currency', type=str, default='BTC', help='Option currency (BTC/ETH)')
    ...
    if config.broadcast_events:
        ...
        print(f"Open 'examples/javascript_client.html' in browser to view volatility surface")
