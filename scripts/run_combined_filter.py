...
class CombinedVolatilityFilter:
    ...
    def initialize(self):
        ...
        if self.config.broadcast_events:
            ...
            logger.info(f"WebSocket server started on ws://{self.config.ws_host}:{self.config.ws_port}")
        ...
        while self.is_running:
            try:
                ...
                if self.futures_filter:
                    ...
                    print(f"  Current volatility: {futures_stats['current_volatility']:.6f}")
                    
                # Option statistics
                if self.config.enable_options and self.option_filter:
                    option_stats = self.option_filter.get_statistics()
                    print(f"\nOptions:")
                    print(f"  Tracked instruments: {option_stats['tracked_instruments']}")
                ...
                if self.broadcast_server:
                    print(f"\nWebSocket Server:")
                    print(f"  Connected clients: {self.broadcast_server.get_client_count()}")
    ...
    def stop(self):
        ...
        if self.broadcast_server and self.futures_filter:
            ...
            if not self.futures_filter.broadcast_server or self.futures_filter.broadcast_server == self.broadcast_server:
...
def main():
    ...
    # Filter parameters
    parser.add_argument('--window', type=int, help='Window size for volatility calculation')
    parser.add_argument('--ar-lag', type=int, help='AR model lag')
    parser.add_argument('--threshold', type=float, help='Volatility threshold')
    parser.add_argument('--optimize', action='store_true', help='Auto-optimize threshold on startup')
    
    # Option parameters
    parser.add_argument('--enable-options', action='store_true', help='Enable option chain monitoring')
    parser.add_argument('--option-currency', type=str, help='Option currency (BTC, ETH)')
    parser.add_argument('--option-expiry-days', type=int, help='Days ahead to track option expiries')
    parser.add_argument('--option-strike-range', type=float, help='Strike range percentage (e.g., 0.25 for ±25%)')
    parser.add_argument('--option-iv-threshold', type=float, help='IV anomaly threshold in std devs')
    
    # Infrastructure parameters
    parser.add_argument('--no-db', action='store_true', help='Disable database storage')
    parser.add_argument('--no-broadcast', action='store_true', help='Disable WebSocket broadcasting')
