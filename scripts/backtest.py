...
def run_backtest(
    days=14, window_size=100, save_to_db=True, db_path="volatility_filter.db"
):
...
def main():
    ...
    parser.add_argument(
        "--days", type=int, default=14, help="Number of days of historical data to use"
    )
    parser.add_argument(
        "--window", type=int, default=100, help="Window size for rolling calculations"
    )
    parser.add_argument(
        "--no-db", action="store_true", help="Do not save results to database"
    )
    parser.add_argument(
    ...
    logger.info(
        f"Starting backtest for {args.days} days with window size {args.window}"
    )
